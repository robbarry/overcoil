...
@MainActor final class OvercoilUITests: XCTestCase {
    ...
    func testDeleteOnlyReadingKeepsCoverAndWatch() {
        ...
        XCTAssertTrue(app.buttons["Delete reading…"].isHittable)
        screenshot("delete-reading-top-action")
        tap("Delete reading…"); screenshot("delete-last-reading-confirmation"); tap("Cancel")
        XCTAssertTrue(app.buttons["Correct entered time"].exists)
        ...
        XCTAssertTrue(app.images["Reference photo for Test watch"].exists)
        screenshot("09-deleted-reading-retained-cover")
    }
    func testDeleteActiveRunCancelsThenRemovesAllReadings() {
        addWatch(); tap("Start timing run"); takePhoto(); enterSeconds("08"); tap("Save reading")
        tap("Add reading"); takePhoto(); enterSeconds("14"); tap("Save reading")
        tap("currentRun")
        XCTAssertTrue(app.buttons["Delete run…"].isHittable)
        screenshot("delete-run-top-action")
        tap("Delete run…")
        XCTAssertTrue(app.staticTexts["Delete this run and all 2 readings?"].waitForExistence(timeout: 5))
        screenshot("delete-run-confirmation")
        tap("Cancel")
        XCTAssertTrue(app.staticTexts["rateValue"].label.contains("+6.0"))
        tap("Delete run…"); tap("Delete run")
        XCTAssertTrue(app.buttons["Start timing run"].waitForExistence(timeout: 5))
        XCTAssertFalse(app.buttons["currentRun"].exists)
        app.terminate(); app.launch()
        XCTAssertTrue(app.images["Reference photo for Test watch"].waitForExistence(timeout: 5))
        XCTAssertFalse(app.staticTexts["watchBoxOverallRate"].exists)
        app.staticTexts["Test watch"].tap()
        XCTAssertTrue(app.buttons["Start timing run"].waitForExistence(timeout: 5))
        screenshot("deleted-run-watch-and-cover-retained")
        tap("Run history")
        XCTAssertTrue(app.staticTexts["No timing runs yet"].waitForExistence(timeout: 5))
    }
    func testDeleteLatestReadingRecalculatesAndKeepsRun() {
        addWatch(); tap("Start timing run"); takePhoto(); enterSeconds("08"); tap("Save reading")
        tap("Add reading"); takePhoto(); enterSeconds("14"); tap("Save reading")
        tap("currentRun")
        app.staticTexts["14 seconds ahead"].tap()
        XCTAssertTrue(app.buttons["Delete reading…"].isHittable)
        tap("Delete reading…"); tap("Delete reading")
        XCTAssertTrue(app.staticTexts["1 reading saved"].waitForExistence(timeout: 5))
        XCTAssertFalse(app.staticTexts["rateValue"].exists)
        XCTAssertTrue(app.staticTexts["8 seconds ahead"].firstMatch.exists)
        screenshot("deleted-latest-reading-offset-not-rate")
        app.terminate(); app.launch()
        app.staticTexts["Test watch"].tap()
        XCTAssertTrue(app.buttons["currentRun"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.staticTexts["1 reading saved"].exists)
        XCTAssertTrue(app.images["Reference photo for Test watch"].exists)
    }
    func testDeleteCompletedRunUpdatesOverallRateAndKeepsActiveRun() {
        addWatch(); tap("Start timing run"); takePhoto(); enterSeconds("08"); tap("Save reading")
        tap("Add reading"); takePhoto(); enterSeconds("14"); tap("Save reading")
        ...
        XCTAssertTrue(app.staticTexts["Completed run"].waitForExistence(timeout: 5))
        tap("Delete run…"); tap("Delete run")
        XCTAssertFalse(app.staticTexts["Completed"].exists)
        XCTAssertTrue(app.staticTexts["Active run"].exists)
        app.terminate(); app.launch()
        XCTAssertTrue(app.staticTexts["watchBoxOverallRate"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.staticTexts["watchBoxOverallRate"].label.contains("+12.0"))
        XCTAssertTrue(app.images["Reference photo for Test watch"].exists)
        screenshot("deleted-completed-run-recalculated-grid")
    }
